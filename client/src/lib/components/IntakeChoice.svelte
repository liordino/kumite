...
<section class="rounded border border-zinc-300 bg-white">
	<header class="border-b border-zinc-300 px-4 py-3">
		<h2 class="text-sm font-semibold text-zinc-900">Intake — Uchikomi</h2>
	...
	<div class="grid gap-px bg-zinc-300 sm:grid-cols-2">
		<div class="bg-white p-4">
			...
			<p class="mt-2 text-sm text-zinc-600">{explanation?.distill.when ?? ''}</p>
			<p class="mt-2 text-sm text-zinc-600">{explanation?.distill.why ?? ''}</p>
			<p class="mt-2 text-xs text-zinc-500">{explanation?.distill.cost ?? ''}</p>
			<button
				type="button"
				class="mt-3 rounded border border-zinc-400 bg-zinc-900 px-3 py-1.5 text-sm font-medium text-white hover:bg-zinc-700 disabled:opacity-50"
		...
		<div class="bg-white p-4">
			...
			<p class="mt-2 text-sm text-zinc-600">{explanation?.skip.when ?? ''}</p>
			<p class="mt-2 text-sm text-zinc-600">{explanation?.skip.why ?? ''}</p>
		...
		<div class="border-t border-zinc-300 p-4">
			...
			{#if showRevert}
				<div class="mt-3 flex items-center gap-3">
					<p class="text-sm text-zinc-600">Restore your original wording and drop the brief?</p>
					<button
						type="button"
						class="rounded border border-red-300 bg-red-50 px-3 py-1.5 text-sm font-medium text-red-800 hover:bg-red-100"
						disabled={busy}
						onclick={revert}
					>
						Revert
	...
	<details class="border-t border-zinc-300 p-4">
		...
		{#if distilled}
			...
			<pre class="max-h-72 overflow-auto whitespace-pre-wrap rounded bg-zinc-50 p-3 text-xs leading-relaxed text-zinc-700">{rawInput}</pre>
			<p class="mt-3 mb-1 text-xs text-zinc-500">Original material (preserved):</p>
			<pre class="max-h-72 overflow-auto whitespace-pre-wrap rounded bg-zinc-50 p-3 text-xs leading-relaxed text-zinc-600">{rawSource}</pre>
		{:else}
			<pre class="max-h-96 overflow-auto whitespace-pre-wrap text-xs leading-relaxed text-zinc-700">{rawInput}</pre>
		{/if}
	</details>
</section>