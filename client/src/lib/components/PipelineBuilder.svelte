<script lang="ts">
	...
	let {
		...
		plan: PipelinePlan;
		busy?: boolean;
		onSave: (plan: PipelinePlan) => Promise<void> | void;
	...
	// .length on null.
	// svelte-ignore state_referenced_locally -- init-capture is the contract: the parent remounts this component via {#key session.pipeline_plan}, so the snapshot is taken once per mount.
...
<section class="rounded border border-stone-300 bg-white">
	...
	<div class="border-b border-stone-300 px-4 py-3">
		...
		<dl class="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-4">
			{#each fourBlock as block (block.label)}
				<div class="rounded border border-stone-300 px-2 py-1.5">
					<dt class="text-xs text-zinc-500">{block.label}</dt>
					<dd class="text-sm font-medium text-stone-800">{block.value}</dd>
				</div>
			{/each}
		</dl>
		{#if working.context.flags.length > 0}
			<ul class="mt-3 space-y-1">
				{#each working.context.flags as flag (flag)}
					<li class="border-l-2 border-amber-400 pl-2 text-sm text-stone-700">{flag}</li>
				{/each}
			</ul>
		{/if}
	</div>
		...
		<div class="border-b border-stone-300 px-4 py-3">
			...
			<ul class="mt-2 space-y-2">
				{#each wave.nodes as node, i (node.id)}
					<li class="flex items-start justify-between gap-3 rounded border border-stone-300 px-3 py-2">
						<label class="flex items-start gap-2">
							<input type="checkbox" class="mt-1" checked={node.enabled} onchange={() => toggle(node)} />
							<span>
								<span class="block text-sm font-medium text-stone-900">{node.display_name}</span>
								<span class="block text-xs text-zinc-500">{node.rationale}</span>
							</span>
						</label>
						<span class="flex shrink-0 items-center gap-1 text-xs">
							{#if i > 0}
								<button type="button" class="rounded border border-stone-300 px-1.5 py-0.5 hover:bg-stone-100" disabled={busy} onclick={() => moveUp(node, wave.nodes, i)}>↑</button>
							{/if}
							<button
								type="button"
								class="rounded border border-stone-300 px-1.5 py-0.5 hover:bg-stone-100"
								disabled={busy}
	...
	<div class="border-b border-stone-300 px-4 py-3">
		...
		{#if showRepo}
			{#if repoError}
				<p class="mt-2 text-sm text-red-700">{repoError}</p>
			...
			{:else}
				<ul class="mt-2 max-h-64 space-y-1 overflow-auto">
					{#each repoAvailable as a (a.agent_id)}
						<li class="flex items-center justify-between gap-2 rounded border border-stone-300 px-2 py-1">
							<span class="text-sm text-stone-800">{a.display_name}</span>
							<span class="flex gap-1 text-xs">
								<button type="button" class="rounded border border-stone-300 px-1.5 py-0.5 hover:bg-stone-100" onclick={() => addRepoAgent(a, 1)}>→ W1</button>
								<button type="button" class="rounded border border-stone-300 px-1.5 py-0.5 hover:bg-stone-100" onclick={() => addRepoAgent(a, 2)}>→ W2</button>
							</span>
						</li>
					{/each}
				</ul>
	...
	<div class="px-4 py-3">
		...
		<ul class="mt-2 space-y-2">
			{#each working.pipeline.fixed as node (node.id)}
				<li class="flex items-start justify-between gap-3 rounded border border-stone-300 bg-white px-3 py-2">
					<span>
						<span class="block text-sm font-medium text-stone-900">{node.display_name}</span>
						<span class="block text-xs text-zinc-500">{node.rationale}</span>
					</span>
