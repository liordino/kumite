...
<article class="border border-zinc-800 bg-zinc-900">
	<header class="flex flex-wrap items-baseline justify-between gap-2 border-b border-zinc-800 px-4 py-2">
		<h3 class="text-sm font-semibold text-zinc-100">{output.display_name}</h3>
		<span class="text-xs text-zinc-500">{statusLabel[output.status] ?? output.status}</span>
	</header>

	<div class="px-4 py-3">
		<p class="text-sm leading-relaxed text-zinc-200">{output.output.summary}</p>
	...
	{#if output.thinking}
		<div class="border-t border-zinc-800 px-4 py-2">
			...
			{#if expanded}
				<pre class="mt-2 max-h-72 overflow-auto whitespace-pre-wrap rounded bg-zinc-900/50 p-3 text-xs leading-relaxed text-zinc-400">{output.thinking}</pre>
			{/if}
		</div>
	{/if}

	{#if output.error}
		<p class="border-t border-zinc-800 px-4 py-2 text-xs text-red-400">{output.error}</p>
	{/if}
</article>