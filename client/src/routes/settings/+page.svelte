...
<div class="mx-auto max-w-3xl space-y-6">
	<section class="rounded border border-zinc-800 bg-zinc-900">
		<header class="border-b border-zinc-800 px-4 py-3">
			<h2 class="text-sm font-semibold text-zinc-100">Inference provider</h2>
		...
		>
			<div>
				<label class="block text-sm font-medium text-zinc-200" for="llm-endpoint">Endpoint</label>
				<input
					id="llm-endpoint"
					type="text"
			...
			<div>
				<label class="block text-sm font-medium text-zinc-200" for="llm-model">Model</label>
				<input
					id="llm-model"
					type="text"
					class="mt-1 w-full rounded border border-zinc-700 px-3 py-2 text-sm focus:border-zinc-700 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-zinc-200" for="llm-api-key">API key</label>
				<input
					id="llm-api-key"
					type="password"
					class="mt-1 w-full rounded border border-zinc-700 px-3 py-2 text-sm focus:border-zinc-700 focus:outline-none"
			...
			{:else if saved}
				<p class="text-sm text-zinc-400">Saved.</p>
			{/if}
			<button
				type="submit"
	...
	<section class="rounded border border-zinc-800 bg-zinc-900">
		<header class="border-b border-zinc-800 px-4 py-3">
			<h2 class="text-sm font-semibold text-zinc-100">Custom agents</h2>
			<p class="mt-1 text-sm text-zinc-400">
			...
			<ul class="divide-y divide-zinc-800">
				{#each customAgents as a (a.id)}
					<li class="flex items-start justify-between gap-3 px-4 py-3">
						<span>
							<span class="block text-sm font-medium text-zinc-100">{a.display_name}</span>
		...
		<form
			class="space-y-3 border-t border-zinc-800 px-4 py-4"
			onsubmit={(e) => {
				e.preventDefault();
				void createAgent();
			}}
		>
			<h3 class="text-sm font-medium text-zinc-200">Define a new agent</h3>
			<div class="grid gap-3 sm:grid-cols-2">
				<div>
					<label class="block text-sm font-medium text-zinc-200" for="agent-name">Name</label>
					<input
						id="agent-name"
						type="text"
						class="mt-1 w-full rounded border border-zinc-700 px-3 py-2 text-sm focus:border-zinc-700 focus:outline-none"
				...
				<div>
					<label class="block text-sm font-medium text-zinc-200" for="agent-wave">Wave preference</label>
					<select
						id="agent-wave"
						class="mt-1 w-full rounded border border-zinc-700 px-3 py-2 text-sm focus:border-zinc-700 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-zinc-200" for="agent-role">Role summary</label>
				<input
					id="agent-role"
					type="text"
					class="mt-1 w-full rounded border border-zinc-700 px-3 py-2 text-sm focus:border-zinc-700 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-zinc-200" for="agent-prompt">System prompt</label>
				<textarea
					id="agent-prompt"
					class="mt-1 h-28 w-full rounded border border-zinc-700 px-3 py-2 font-mono text-sm focus:border-zinc-700 focus:outline-none"
					bind:value={newAgent.system_prompt}
				></textarea>
			</div>
			{#if agentError}
				<p class="text-sm text-red-400">{agentError}</p>
			{/if}
			<button
				type="submit"
				class="rounded border border-zinc-700 px-3 py-1.5 text-sm font-medium text-zinc-200 hover:bg-zinc-900"
			>
				Add agent
			</button>
		</form>
