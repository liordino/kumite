...
<div class="mx-auto max-w-3xl space-y-6">
	<section class="rounded-none border border-stone-300 bg-white">
		...
		>
			<div>
				<label class="block text-sm font-medium text-stone-800" for="llm-endpoint">Endpoint</label>
				<input
					id="llm-endpoint"
					type="text"
			...
			<div>
				<label class="block text-sm font-medium text-stone-800" for="llm-model">Model</label>
				<input
					id="llm-model"
					type="text"
					class="mt-1 w-full rounded border border-stone-300 px-3 py-2 text-sm focus:border-stone-300 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-stone-800" for="llm-api-key">API key</label>
				<input
					id="llm-api-key"
					type="password"
					class="mt-1 w-full rounded border border-stone-300 px-3 py-2 text-sm focus:border-stone-300 focus:outline-none"
			...
			{:else if saved}
				<p class="text-sm text-stone-500">Saved.</p>
			{/if}
			<button
				type="submit"
	...
	<section class="rounded-none border border-stone-300 bg-white">
			...
			<ul class="divide-y divide-stone-200">
				{#each customAgents as a (a.id)}
					<li class="flex items-start justify-between gap-3 px-4 py-3">
						<span>
							<span class="block text-sm font-medium text-stone-900">{a.display_name}</span>
		...
		<form
			class="space-y-3 border-t border-stone-300 px-4 py-4"
			onsubmit={(e) => {
				e.preventDefault();
				void createAgent();
			}}
		>
			<h3 class="text-sm font-medium text-stone-800">Define a new agent</h3>
			<div class="grid gap-3 sm:grid-cols-2">
				<div>
					<label class="block text-sm font-medium text-stone-800" for="agent-name">Name</label>
					<input
						id="agent-name"
						type="text"
						class="mt-1 w-full rounded border border-stone-300 px-3 py-2 text-sm focus:border-stone-300 focus:outline-none"
				...
				<div>
					<label class="block text-sm font-medium text-stone-800" for="agent-wave">Wave preference</label>
					<select
						id="agent-wave"
						class="mt-1 w-full rounded border border-stone-300 px-3 py-2 text-sm focus:border-stone-300 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-stone-800" for="agent-role">Role summary</label>
				<input
					id="agent-role"
					type="text"
					class="mt-1 w-full rounded border border-stone-300 px-3 py-2 text-sm focus:border-stone-300 focus:outline-none"
			...
			<div>
				<label class="block text-sm font-medium text-stone-800" for="agent-prompt">System prompt</label>
				<textarea
					id="agent-prompt"
					class="mt-1 h-28 w-full rounded border border-stone-300 px-3 py-2 font-mono text-sm focus:border-stone-300 focus:outline-none"
					bind:value={newAgent.system_prompt}
				></textarea>
			</div>
			{#if agentError}
				<p class="text-sm text-red-700">{agentError}</p>
			{/if}
			<button
				type="submit"
				class="rounded border border-stone-300 px-3 py-1.5 text-sm font-medium text-stone-800 hover:bg-stone-100"
			>
				Add agent
			</button>
		</form>
